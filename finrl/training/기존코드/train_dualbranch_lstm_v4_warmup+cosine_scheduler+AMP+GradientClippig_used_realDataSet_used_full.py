...
from finrl.loss_functions.warmup_cosine_scheduler_realData import WarmupCosineAnnealingScheduler

from datetime import datetime


print("x_minute shape check:", np.load("data/dualbranch/SOLUSDT_1m_finrl_20250101~20250426_X_minute_full.npy").shape)

print(np.load('data/dualbranch/SOLUSDT_1m_finrl_20250101~20250426_X_minute_full.npy').dtype)
...
# ✅ Trainer Function
def train_one_epoch(model, dataloader, optimizer, scheduler, combined_loss_fn, scaler, device="cuda", max_grad_norm=1.0):
    model.train()
...
if __name__ == "__main__":
    ...
    dataset = RealDualBranchDataset(data_dir)
    dataloader = DataLoader(dataset, batch_size=2014, shuffle=True)

    # ✅ 학습 시작
    avg_loss, avg_entry_loss, avg_direction_loss = train_one_epoch(
        model, dataloader, optimizer, scheduler, combined_criterion, scaler, device
    )
...
# (기존 Dataset과 Model 정의도 그대로)

if __name__ == "__main__":
    device = "cuda" if torch.cuda.is_available() else "cpu"

    dataset = RealDualBranchDataset(data_dir="data/dualbranch/")
    dataloader = DataLoader(dataset, batch_size=32, shuffle=True)

    model = DualBranchLSTMModel(input_size_minute=17, input_size_daily=20).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    scheduler = WarmupCosineAnnealingScheduler(optimizer, warmup_steps=10, total_steps=100)
    scaler = GradScaler()

    loss_fn_entry = nn.BCEWithLogitsLoss()
    loss_fn_direction = nn.CrossEntropyLoss()

    num_epochs = 1

    os.makedirs("testlog", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = f"testlog/train_log_{timestamp}.txt"

    with open(log_path, "w") as log_file:
        for epoch in range(num_epochs):
            model.train()
            for batch_idx, (x_minute, x_daily, y_entry, y_direction) in enumerate(dataloader):
                x_minute = x_minute.to(device)
                x_daily = x_daily.to(device)
                ...
                with autocast():
                    entry_preds, direction_preds = model(x_minute, x_daily)
                    entry_loss = loss_fn_entry(entry_preds.view(-1), y_entry.view(-1))
                    direction_loss = loss_fn_direction(direction_preds, y_direction)
                    total_loss = entry_loss + direction_loss
                ...
                log_text = f"Epoch {epoch} Batch {batch_idx}: Total Loss: {total_loss.item():.4f}, Entry Loss: {entry_loss.item():.4f}, Direction Loss: {direction_loss.item():.4f}\n"
