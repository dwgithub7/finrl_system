...
class TQDMCallback(Callback):
    ...
    def on_epoch_end(self, epoch_idx, logs=None):  # 🔥 수정: logs=None 추가
        self.pbar.update(1)
...
class TabNetTrainer:
    ...
    def train(self, X, entry_y, direction_y):
        ...
        # 샘플링: 빠른 테스트용 (최대 1000개)
        if len(X) > 1000:
            indices = np.random.choice(len(X), 10000, replace=False)
            X = X[indices]
            entry_y = entry_y[indices]
            direction_y = direction_y[indices]
        ...
        self.entry_model.fit(
            ...
            patience=10,
            batch_size=2096,
            virtual_batch_size=1025,
            num_workers=6,
            drop_last=False,
            max_epochs=50,
            callbacks=[TQDMCallback(total_epochs=50)]
        )
