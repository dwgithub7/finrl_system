...
def train_one_epoch(model, dataloader, optimizer, scheduler, combined_loss_fn, scaler, device="cuda", max_grad_norm=1.0):
    ...
    for batch in dataloader:
        ...
        x_minute, x_daily = x_minute.to(device), x_daily.to(device)
        entry_labels, direction_labels = entry_labels.to(device), direction_labels.to(device)
...
if __name__ == "__main__":
    ...
    avg_loss, avg_entry_loss, avg_direction_loss = train_one_epoch(model, dataloader, optimizer, scheduler, combined_criterion, scaler, device)
