...
# 7. DRL Agent 및 PPO 모델 생성 (GPU 최적화)
agent = DRLAgent(env=train_env)
ppo_params = {
    "n_steps": 4096,           # 크게 늘려서 효율 향상
    "ent_coef": 0.005,         # 탐색 증가
    "learning_rate": 0.0001,   # 안정적 학습
    "batch_size": 1024,        # 대형 배치로 GPU 병렬화 강화
    "device": "cuda"           # ✅ GPU 사용
}
model = agent.get_model("ppo", model_kwargs=ppo_params)

# 8. 모델 학습
trained_model = agent.train_model(model=model, tb_log_name="ppo_gpu", total_timesteps=50000)
